...
public class TreeUtils {
     ...
     * @return 树形结构列表
     */
    public static <T extends TreeModel<T>> List<T> buildTree(List<T> allDepts, T root) {
     ...
     * @return 树形结构列表
     */
    public static <T extends TreeModel<T>> List<T> buildTree(List<T> allDepts, T root, Object excludeNodeId) {
        List<T> trees = new ArrayList<>();
        // 构建树形结构
     ...
     * @param <T>      树节点类型
     */
    private static <T extends TreeModel<T>> void constructTreeRecursive(T parent, List<T> allDepts) {
        // 遍历所有部门
        for (T dept : allDepts) {
     ...
     * @param <T>           树节点类型
     */
    private static <T extends TreeModel<T>> void constructTreeRecursiveExcludeNode(T parent, List<T> allDepts, Object excludeNodeId) {
        // 遍历所有节点
        for (T dept : allDepts) {
            if (parent.getChildren() == null) {
                parent.setChildren(new ArrayList<>());
     ...
     * @return 根节点
     */
    public static <T extends TreeModel<T>> T getRoot(Class<T> clazz) {
        try {
